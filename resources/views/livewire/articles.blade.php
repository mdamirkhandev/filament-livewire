        ...
        <div class="container">
            <div class="row">
                <div class="col-8 mx-auto text-center">
                    <h2 class="mb-3 text-capitalize">Blogs</h2>
                    <ul class="list-inline breadcrumbs text-capitalize" style="font-weight:500">
                        <li class="list-inline-item"><a wire:navigate href="{{ route('home') }}">Home</a>
                        </li>
                        <li class="list-inline-item">/ &nbsp; <a wire:navigate href="{{ route('blog') }}">Blogs</a>
                        </li>
                    </ul>
                </div>
            </div>
                                    ...
                                    <article class="blog-post">
                                        <div class="post-slider slider-sm rounded">
                                            <img loading="lazy" decoding="async"
                                                src="{{ asset('storage/' . $article->image) }}" alt="Post Thumbnail">
                                        </div>
                                        <div class="pt-4">
                                            <p class="mb-3">
                                                {{ \Carbon\Carbon::parse($article->created_at)->format('d M Y') }}</p>
                                            <h2 class="h4"><a class="text-black" wire:navigate
                                                    href="{{ route('blog-details', $article->id) }}">{{ $article->title }}</a>
                                            </h2>
                                            <a wire:navigate href="{{ route('blog-details', $article->id) }}"
                    ...
                    <div class="widget widget-categories">
                        ...
                        <ul class="list-unstyled widget-list">
                            @foreach ($categories as $category)
                                <li><a wire:navigate
                                        href="{{ route('blog') . '?category=' . $category->slug }}">{{ $category->name }}
                                    ...
                                    <a class="text-black" href="/blog/elements/">
                                        <div class="widget-post-image flex-shrink-0 me-3">
                                            <img class="rounded" loading="lazy" decoding="async"
                                                src="{{ asset('storage/' . $latestArticle->image) }}"
                                                alt="Post Thumbnail">
                                        </div>
                                    </a>
                                    <div class="flex-grow-1">
                                        <h5 class="h6 mb-0"><a class="text-black"
                                                href="blog-details.html">{{ $latestArticle->title }}</a></h5>
                                        <small>
